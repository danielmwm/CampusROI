...
function DollarInput({ id, value, onChange, label, hint, suffix = '/yr' }) {
  ...
  useEffect(() => {
    // Sync only when value changes from outside (parent reset, program change, etc.)
    if (value !== internalVal.current) {
  ...
  function handleChange(e) {
    ...
    const str = e.target.value.replace(/[^0-9]/g, '').replace(/^0+(?=\d)/, '');
    setRaw(str);
    const num = parseInt(str, 10) || 0;
    internalVal.current = num;
    onChange(num);
  }
