...
const PROVINCES = ['Ontario', 'British Columbia', 'Quebec', 'Alberta', 'Manitoba', 'Saskatchewan', 'Nova Scotia', 'New Brunswick', 'Newfoundland and Labrador'];
const CITIES = ['Toronto', 'Vancouver', 'Montreal', 'Calgary', 'Edmonton', 'Ottawa', 'Waterloo/KW', 'Victoria', 'Winnipeg', 'Saskatoon', 'Halifax', 'London', 'Hamilton', 'Kingston', 'Quebec City', "St. John's", 'Fredericton', 'Guelph', 'Windsor', 'Oshawa'];
const GRAD_YEARS = [2026, 2027, 2028, 2029, 2030];
const COLORS = [
  '#0F172A', '#22C55E', '#3B82F6', '#F59E0B', '#EF4444',
...
function ScenarioColumn({ index, scenario, sharedInputs, onScenarioChange, onRemove, canRemove }) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-md p-4 space-y-3 w-64 shrink-0">
      <div className="flex items-center justify-between">
        ...
        {canRemove && (
          <button onClick={onRemove} className="text-slate-300 hover:text-red-400 transition-colors p-0.5 rounded" title="Remove">
...
export default function Compare() {
  ...
  const [scenarios, setScenarios] = useState([{ ...DEFAULT_SCENARIO }, { ...DEFAULT_SCENARIO }]);
  ...
  function addScenario() {
    if (scenarios.length < MAX_SCENARIOS) setScenarios((prev) => [...prev, { ...DEFAULT_SCENARIO }]);
  }
    ...
    <div className="flex-1 bg-neutral-50">
      ...
      <div className="bg-black">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          ...
          <h1 className="text-3xl md:text-4xl font-light text-white tracking-tight">Compare Programs</h1>
          <p className="text-sm font-light text-white/50 mt-2 max-w-lg">Side-by-side comparison of up to 10 programs on the same chart — net cost, salary, break-even, and 10-year net worth.</p>
        </div>
      </div>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex gap-3 overflow-x-auto pb-2 mb-4 scrollbar-hide">
          ...
          {scenarios.length < MAX_SCENARIOS && (
            <button
              onClick={addScenario}
              className="w-64 shrink-0 border-2 border-dashed border-slate-200 rounded-md flex flex-col items-center justify-center gap-2 hover:border-slate-300 hover:bg-white transition-colors min-h-[140px] group"
            >
              <div className="w-8 h-8 rounded-full border-2 border-slate-200 group-hover:border-slate-400 flex items-center justify-center transition-colors">
              ...
              <p className="text-xs font-medium text-slate-400 group-hover:text-slate-500 transition-colors">Add program</p>
              <p className="text-[10px] text-slate-300">{scenarios.length}/{MAX_SCENARIOS}</p>
            </button>
          )}
        </div>
        ...
        {hasResults ? (
          <>
            ...
            {/* Comparison table */}
            <div className="bg-white border border-slate-200 rounded-md overflow-x-auto mb-4">
