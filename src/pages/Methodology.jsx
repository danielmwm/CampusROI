function Section({ title, children }) {
  return (
    <div className="mb-14">
      <h2 className="text-2xl font-semibold text-black mb-4 pb-3 border-b border-neutral-150">{title}</h2>
      {children}
    </div>
  );
}
...
function Formula({ name, formula, explanation }) {
  return (
    <div className="bg-slate-50 rounded-2xl p-6 mb-4">
      <h4 className="text-sm font-bold text-black mb-2">{name}</h4>
    ...
    <div className="flex-1 bg-white">
      ...
      <div className="bg-black">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <p className="text-[11px] font-medium text-white/40 uppercase tracking-[0.25em] mb-2">How It Works</p>
          <h1 className="text-3xl md:text-4xl font-light text-white tracking-tight mb-4">Data Methodology</h1>
          <p className="text-base font-light text-white/50 leading-relaxed max-w-2xl mb-8">
            Every number in CampusROI comes from a real, citable source. We individually researched all 25 universities across 9 provinces — tuition from each registrar, co-op earnings from institutional reports, and salaries from national labour surveys.
          </p>
          <div className="flex flex-wrap gap-3">
            {[
              { icon: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z', label: 'Last updated: January 2025' },
              { icon: 'M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4', label: '25 universities researched' },
              { icon: 'M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7', label: '20 cities covered' },
            ].map(({ icon, label }) => (
              <div key={label} className="inline-flex items-center gap-2 text-xs text-white/40 bg-white/[0.07] border border-white/10 px-4 py-2 rounded-full">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
            {[
              ...
              <div key={city} className="bg-slate-50 rounded-lg px-3 py-2.5 flex items-center justify-between">
                ...
                <span className="text-xs font-bold text-black">{cost}</span>
              </div>
            ))}
          </div>
        </Section>
        ...
        {/* Update cadence */}
        <div className="bg-neutral-50 rounded-2xl p-6 text-sm text-slate-500 border border-neutral-100">
          <p className="font-semibold text-black mb-1.5">Data Review Cadence</p>
